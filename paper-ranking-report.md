...
## The arXiv weekly-scan batch (added 2026-09-07)

Five preprints pulled from a `sortBy=submittedDate` arXiv scan for the trailing week (2026-08-31 through 2026-09-07), filtered down from ~45 sports-tagged submissions to the ones with a genuine sports-analytics methods contribution (the scan also turns up humanoid-robot soccer/football control papers, video-commentary LLM benchmarks, and unrelated physics papers that happen to use "hockey-stick" as a shape descriptor — none of those are in this batch). CSV 286 → 291 rows. All five are **machine conversions** (`pdftotext` + scripted reflow, headed by an explicit provenance comment); equations, tables and figures are not preserved — consult the PDF for those.

### ELASTIC: Trajectory-Based Synchronization of Event and Tracking Data in Soccer — 22/30, highest of the batch

Kim, Choi, Lee, Seo, Boomstra, Yoon & Park (KAIST / AFC Ajax / Fitogether), [arXiv:2608.30227](https://arxiv.org/abs/2608.30227), submitted 2026-08-31, to appear at CIKM '26.

The problem this fixes is one every soccer-analytics practitioner who has combined event and tracking feeds has hit: event timestamps are human-annotated and imprecise, so naively joining them to tracking frames puts the "pass" event several frames before or after the ball actually left the passer's foot, corrupting any spatiotemporal feature built on top. ELASTIC (**E**vent-**L**ocation-**A**gno**ST**ic synchron**I**zer) infers event start/end timestamps purely from player and ball trajectories — never touching the noisy annotated event locations — by inserting virtual termination events to recover otherwise-invisible ball receptions, extracting physically-plausible ball-touch candidate frames, then aligning the event sequence against those candidates with an extended Needleman–Wunsch (the sequence-alignment algorithm from bioinformatics). Reproducibility 7: code and a purpose-built ground-truth benchmark are public (`github.com/hyunsungkim-ds/elastic`), built by hand-annotating the **Sportec Open DFL Dataset** — genuinely open, unlike almost every other tracking dataset in this collection — though the full-scale downstream-task experiments lean on a second, proprietary Eredivisie tracking set because Sportec alone is only seven matches. This is exactly the kind of unglamorous data-engineering paper this repository under-collects relative to modeling papers, despite being the thing a working analyst is most likely to actually need.

### Dummy RAPM: Representing Low-Minute Players in Regularized Adjusted Plus-Minus — 21/30

Watts, Pipping-Gamón & Wyner (Wharton), [arXiv:2608.19454](https://arxiv.org/abs/2608.19454), submitted 2026-08-19, revised 2026-08-21.

A small, surgical fix to a gap every public RAPM implementation shares: when low-minute players are filtered out of the design matrix to control noise, their stints stay in the training data but the model no longer knows those stints had a weaker player on the floor — the omitted information silently leaks onto whichever retained players are correlated with it. Dummy RAPM adds ten pooled count regressors (five "N excluded players on the home side," five for away) with their own ridge penalty, tuned by chronological validation to a 10-minute threshold and a 2.2 dummy-to-player penalty ratio. Across 16 NBA seasons (19,589 games, 496,575 stints) the fix drops held-out game-margin RMSE from 12.897 to 12.856 — small (0.30%) but consistent, 13 of 16 seasons, paired t-test p = 0.0068 — and an unfiltered-benchmark check confirms the gain is specifically from restoring the count information, not from the filtering itself. Practicality 7: this is a drop-in addition to any ridge-RAPM pipeline, not a new framework. Reproducibility 9 — the strongest in this batch and one of the strongest in the whole collection: built entirely on **public ESPN play-by-play distributed through the sportsdataverse project**, with code at `github.com/whartonsabi/dummy-rapm`. Worth flagging for this repository specifically — the data pipeline it cites (`Gilani, S. (2026). hoopR: Access Men's Basketball Play-by-Play Data`) is this repo owner's own package, so this is a paper from outside the SportsDataverse community building directly on `hoopR` infrastructure. Worth a pointer from the sportsdataverse/hoopR side (README, changelog, or a "used by" note) back to this application.

### Auditing Contextual Bias in Human Ball-Strike Calls Using KBO's Automated Umpiring Transition — 18/30

Lee & Ko (Yonsei), [arXiv:2609.03786](https://arxiv.org/abs/2609.03786), submitted 2026-09-03.

The design is the interesting part: rather than trying to prove human umpire bias directly from human-only data — where context (count, inning, batter reputation) is confounded with pitch selection and can't be cleanly separated — the paper uses the KBO's 2024 switch to an Automated Ball-Strike system as a natural-experiment benchmark. It fits a boundary-aware logistic model (signed distance to the rule-zone edge, not a rectangular lookup) on 1.2M pitches spanning 2021 through partial 2026, comparing effect sizes in the 2022–23 human-umpire era against the 2024+ ABS era. The headline result is a clean before/after: in the 0.25-ft boundary band, a 0–2 count carried a −17.17 percentage-point strike-call effect and 3–0 a +6.61-point effect under humans, both collapsing to statistically indistinguishable from zero under ABS. Late-inning/close-game strike-proneness and catcher-associated residual variation show the same pre/post pattern; home-team advantage and salary-based reputation proxies are weaker or null. This directly extends the same group's own 2025 *Scientific Reports* case study and generalizes the pitch-framing literature already in this collection (Deshpande & Wyner's hierarchical Bayesian framing model is a direct citation). Novelty 7 for the audit design; practicality 6 — most actionable for rules committees and leagues evaluating an ABS transition (MLB included) rather than a team analytics group; reproducibility 5 — data is public (scraped from Naver Sports) and the modeling is fully specified, but "cleaned derived tables and replication scripts will be released subject to data-source terms of use" is a promise, not a repository, as of this scan.

### The Traveling Tournament Problem: An Overview — 18/30

Van Bulck, Yang, Goossens & Trick, [arXiv:2609.03612](https://arxiv.org/abs/2609.03612), submitted 2026-09-03.

Not a new result — a survey, so novelty is deliberately scored low (4) under this report's own definition — but it is the first comprehensive literature review of the Traveling Tournament Problem in 25 years, and it comes with real infrastructure attached: the authors have taken over maintenance of the RobinX benchmark-instance and best-known-bounds repository originally built by the late Michael Trick (a co-author here), tracking and validating lower/upper bounds across the field's standard test instances. The TTP is the double-round-robin, minimize-total-travel scheduling problem that originated from MLB's own 1990s scheduling headaches — the direct operations-research descendant of the 1971 Carter & Machol paper already at the top of this collection's Top 25. Practicality 8: anyone building or evaluating a sports-scheduling optimizer now has one place to find problem variants, benchmark instances, and current state-of-the-art bounds instead of reconstructing 25 years of literature by hand. Reproducibility 6 — no standalone empirical result to rebuild, but the maintained public bounds repository is itself a live, checkable resource.

### Unified Pitch Graphs for Diagnosing Pitching Strategy — 18/30

Lee & Ko (Yonsei), [arXiv:2609.03810](https://arxiv.org/abs/2609.03810), submitted 2026-09-03.

A hierarchical graph representation for pitch-sequencing analysis that keeps every pitch as an exact physical event (reconstructed 3D trajectory, not a coarse pitch-type bucket) while organizing the same events at multiple semantic/temporal resolutions, with a support-adaptive backoff that falls back to coarser or shorter representations only when a fine-grained pattern lacks statistical support. On 3.94 million public MLB Statcast pitches (2021 through partial 2026), the backoff mechanism lifts held-out path coverage from 18.9% to 94.9% while improving execution-reconstruction fit from R² = 0.495 to 0.685, and the authors show it localizes controlled pitcher execution changes that discrete pitch-mix or Markov-chain sequence representations miss entirely. Same authors and same natural-experiment instinct as the KBO umpiring paper above — comfortable working from large public tracking corpora (Statcast here, KBO/Naver there) rather than proprietary optical data. Reproducibility 5: Statcast/Baseball Savant data is public and well-documented, but no code repository is given in the paper as scanned.

### What this batch adds to the collection

The strongest theme is data-pipeline work rather than new outcome metrics: ELASTIC and Dummy RAPM are both plumbing fixes for problems every practitioner using event/tracking or RAPM data already has, and both score high specifically because the fix, the data, and the code are all public. The KBO and Statcast pitch-sequencing papers share an author pair and a research style — public, large-scale tracking corpora used as natural-experiment or descriptive infrastructure rather than headline-metric papers — that this collection has comparatively little of; MLB Statcast and KBO/Naver pitch data are both far more open than optical basketball or soccer tracking, and it shows in their reproducibility scores relative to the SportVU-era NBA papers in "the proprietary-data trap" above. The TTP survey is the odd one out — no new empirical result — but it is the collection's first operations-research literature review and pairs naturally with the 1971 Carter & Machol paper and the Colley Matrix paper already at the top of the Top 25.
